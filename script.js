...
const syncLive2dToggleUI=()=>{if(!live2dToggleButton)return;const label=ui("live2dSwitch")||ui("live2dToggle")||"Live2D";const stateOn=ui("live2dOn")||(currentLanguage==="en"?"On":"开启");const stateOff=ui("live2dOff")||(currentLanguage==="en"?"Off":"关闭");const state=live2dEnabled?stateOff:stateOn;live2dToggleButton.classList.toggle("active",live2dEnabled);live2dToggleButton.setAttribute("aria-pressed",String(live2dEnabled));const sep=currentLanguage==="en"?": ":"：";live2dToggleButton.setAttribute("aria-label",`${label}${sep}${state}`);live2dToggleButton.setAttribute("title",`${label}${sep}${state}`);live2dToggleButton.textContent=state};
...
const applyLanguage=(mode)=>{
  ...
  const projectTitles=document.querySelectorAll(".project-title-btn"),projectTitleCopy=[copy.projectLifeTitle,copy.projectQuotesTitle,copy.projectKindleTitle,copy.projectYoutubeTitle,copy.projectGithubTitle,copy.projectPcGuardTitle,copy.projectAnimeTitle],projectAriaCopy=[copy.projectLifeAria,copy.projectQuotesAria,copy.projectKindleAria,copy.projectYoutubeAria,copy.projectGithubAria,copy.projectPcGuardAria,copy.projectAnimeAria];projectTitles.forEach((el,index)=>{if(projectTitleCopy[index]){el.textContent=projectTitleCopy[index];el.setAttribute("aria-label",projectAriaCopy[index])}});
  const projectDescriptions=document.querySelectorAll(".project-grid .card p");[copy.projectLifeDesc,copy.projectQuotesDesc,copy.projectKindleDesc,copy.projectYoutubeDesc,copy.projectGithubDesc,copy.projectPcGuardDesc,copy.projectAnimeDesc].forEach((text,index)=>{if(projectDescriptions[index])projectDescriptions[index].textContent=text});
  ...
  setAttr("#project-speed-toggle","aria-label",copy.adjustScrollSpeed);setAttr("#project-speed-toggle","title",copy.scrollSpeed);const speedLabel=document.querySelector("#project-speed-panel label[for='project-speed-slider']");if(speedLabel)speedLabel.textContent=copy.scrollSpeed;setAttr("#project-speed-slider","aria-label",copy.adjustProjectScrollSpeed);
