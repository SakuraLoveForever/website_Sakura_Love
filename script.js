...
const updateMusicButtonState=()=>{if(!pageMuteToggleButton)return;const active=Boolean(player.src)&&!player.paused&&!isPageMuted;pageMuteToggleButton.classList.toggle("is-playing",active);pageMuteToggleButton.classList.toggle("muted",isPageMuted||!active);pageMuteToggleButton.title=isPageMuted?ui("unmute"):ui("mute")};
...
const updateSidebarActive=(targetId=null)=>{
  ...
  const current=targetId||sections.reduce((active,section)=>section.getBoundingClientRect().top<=window.innerHeight*0.86?section.id:active,"hero");
...
applySidebarAvatar(localStorage.getItem("sidebarAvatar")||"");applyStyle(localStorage.getItem("stylePreset")||"apple");applyLayoutMode(localStorage.getItem("layoutMode")||"desktop");applyLanguage(currentLanguage);controls();playModeUI();updateSidebarActive();
...
sidebarLinks.forEach(link=>link.addEventListener("click",()=>{const href=link.getAttribute("href");if(href?.startsWith("#"))updateSidebarActive(href.slice(1))}));
...
if(musicToggle)musicToggle.addEventListener("change",()=>withViewportPreserved(()=>{musicEnabled=musicToggle.checked;localStorage.setItem("musicEnabled",String(musicEnabled));if(musicEnabled){clearInterval(bgTimer);if(isViewerEnabled())showScene(currentRole,{withMusic:true})}else{player.pause();scheduleBgOnly()}showToast(musicEnabled?ui("musicOn"):ui("musicOff"))}));
...
if(headerMusicNextButton)headerMusicNextButton.addEventListener("click",()=>withViewportPreserved(()=>{playNext();showToast(bgPlayMode==="single"?ui("songChanged"):ui("roleChanged"))}));
if(pageMuteToggleButton)pageMuteToggleButton.addEventListener("click",()=>withViewportPreserved(()=>{isPageMuted=!isPageMuted;player.muted=isPageMuted;localStorage.setItem("pageMuted",String(isPageMuted));controls();showToast(isPageMuted?ui("muted"):ui("unmuted"))}));
