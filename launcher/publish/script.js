...
const syncLive2dToggleUI=()=>{if(!live2dToggleButton)return;const label=currentLanguage==="en"?"Live2D mascot":"看板娘";const state=live2dEnabled?(currentLanguage==="en"?"On":"开启"):(currentLanguage==="en"?"Off":"关闭");live2dToggleButton.classList.toggle("active",live2dEnabled);live2dToggleButton.setAttribute("aria-pressed",String(live2dEnabled));live2dToggleButton.setAttribute("aria-label",currentLanguage==="en"?`${label}: ${state}`:`${label}：${state}`);live2dToggleButton.setAttribute("title",currentLanguage==="en"?`${label}: ${state}`:`${label}：${state}`);live2dToggleButton.textContent=state};
...
const refreshLive2dAfterShow=()=>{
  const settle=()=>settleLive2dChrome({frames:4});
  if(!live2dEnabled)return;
  if(!live2dInitStarted){
    ensureLive2dInitialized();
    settle();
    return;
  }
...
const initLive2d=async()=>{
  ...
  try{
    ...
    live2dWidget.addEventListener("pointerenter",()=>showLive2dSettingsButton());
    live2dWidget.addEventListener("pointermove",e=>{
      if(isLive2dSettingsTarget(e.target))return;
      if(isAroundLive2dWidget(e,{wide:true}))showLive2dSettingsButton();
    });
